# `file_for_Example_Extended.jl`

**Proves:** depth 1/2/3; `,`/`&&`/`!` conditions; `#~!`; inline `#~ hide`/`#~ discard`/`#~`; blank-line detach; inheritance (file-level rule → code + markdown); order-of-application; implicit close; `hide`=`## ` / `discard`=omit / default=show
...
Committed pair: `examples/InFileFolder/file_for_Example_Extended.jl` → `examples/OutFileFolder/file_for_Example_Extended.jl` — the same
